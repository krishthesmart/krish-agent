...
from krish_agent.tools import read_file, write_file, run_command, parse_json_response
from krish_agent.prompts import REVIEWER_SYSTEM_PROMPT, REVIEWER_TASK_PROMPT_TEMPLATE


class ReviewerAgent:
    """Senior reviewer using Devstral model."""

    def __init__(self, model_endpoint: str = "http://localhost:11434", model_name: str = None):
        """
        Initialize the reviewer agent.

        Args:
            model_endpoint: URL of the Ollama API endpoint (default: http://localhost:11434).
            model_name: Name of the model to use. If None, user will be prompted to select from available models.
        """
    ...
    def _select_model(self) -> str:
        """Let user choose a model from available Ollama models with TUI."""
        import requests
        ...
        try:
            response = requests.get(f"{self.model_endpoint}/api/tags")
            models = response.json().get("models", [])

            if not models:
                console.print(Panel("[red]✗ No models found in Ollama[/red]", title="Error"))
                console.print("Make sure Ollama is running: [cyan]ollama serve[/cyan]")
                return "mistral:latest"

            # Create beautiful table
            table = Table(title="[bold cyan]Available Ollama Models[/bold cyan]", show_header=True, header_style="bold magenta")
            ...
            for i, model in enumerate(models, 1):
                ...
                table.add_row(str(i), model['name'], f"{size_gb:.1f} GB", family)

            console.print(table)

            try:
                choice = Prompt.ask("[bold cyan]Select a model[/bold cyan]", choices=[str(i) for i in range(1, len(models) + 1)])
                selected = models[int(choice) - 1]["name"]
                console.print(Panel(f"[green]✓ Using reviewer model: {selected}[/green]", title="Success"))
                return selected
            except (ValueError, IndexError):
                console.print(Panel("[red]✗ Invalid selection[/red]"))
                return "mistral:latest"

        except requests.exceptions.ConnectionError:
            console.print(Panel("[red]✗ Cannot connect to Ollama[/red]", title="Error"))
            console.print(f"Make sure Ollama is running at: [cyan]{self.model_endpoint}[/cyan]")
            console.print("Command: [cyan]ollama serve[/cyan]")
            return "mistral:latest"
